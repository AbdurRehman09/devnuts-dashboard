...
const meetingValidation = [
  body('title').trim().isLength({ min: 1 }).withMessage('Title is required'),
  body('meetingDate').optional().isISO8601().withMessage('Invalid meeting date format'),
  ...
  body('participants.*').optional().trim().isLength({ min: 1 }).withMessage('Participant name/email is required')
];
...
const projectValidation = [
  ...
  body('teamMembers.*').optional().trim().isLength({ min: 1 }).withMessage('Team member name is required')
];
